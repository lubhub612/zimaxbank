...
export default function MultiplyArea() {
  ...
  const viewStakeZIMAX = async () => {
    try {
     ...
     str+=myString1;
     let myStringArray0 = myString1.split(',');
     ...
     for(let i=0; i < myStringArrayStr.length; i+=1) {
      ...
      let date1 = new Intl.DateTimeFormat('en-US', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit' }).format( myStringArrayStr[i+2]);
      let date2 = new Intl.DateTimeFormat('en-US', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit' }).format( myStringArrayStr[i+3])
        result.push({amount: (myStringArrayStr[i]/(10 ** 18)), lock_period: pstr, lock_time : myStringArrayStr[i+2], unlock_time : myStringArrayStr[i+3], reward : (myStringArrayStr[i+4]/ (10 ** 18))})
       }
     }
     console.log(result);
